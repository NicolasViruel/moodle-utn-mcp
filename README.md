...
Local stdio MCP server for the approved UTN Moodle origins `https://tup.sied.utn.edu.ar` and `https://utnsannicolas.quinttos.com`. It offers a user-operated, visible-browser login at the Quinttos login page and one read-only tool for the signed-in user's own rendered Moodle profile.

## Scope and safety

- The only targets are the hard-coded `https://tup.sied.utn.edu.ar` and `https://utnsannicolas.quinttos.com` origins. No tool accepts a URL or origin input. The browser context blocks every request to another origin.
...
- The only authenticated read is `moodle_read_my_profile`. It follows the signed-in user's rendered user-menu profile link and returns visible profile fields, including career-related fields if Moodle renders them.
